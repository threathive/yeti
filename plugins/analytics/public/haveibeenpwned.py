...
class HaveIBeenPwnedAPI(object):
    ...
    def search_breaches(observable):
        if isinstance(observable, Email):
            url = '{api}breachedaccount/{account}'.format(api=HaveIBeenPwnedAPI.API, account=observable.value)
            try:
                ...
                logging.error('Exception while getting email report {}'.format(e.message))
                return None
    ...
    def search_pastes(observable):
        if isinstance(observable, Email):
            url = '{api}pasteaccount/{account}'.format(api=HaveIBeenPwnedAPI.API,account=observable.value)
